<?php

namespace Database\Factories;

use App\Models\Penilaian;
use App\Models\Dosen;
use App\Models\Proposal;
use Illuminate\Database\Eloquent\Factories\Factory;

/**
 * Factory untuk model Penilaian (Penilaian Proposal).
 *
 * Menghasilkan data uji yang realistis (locale Bahasa Indonesia) untuk
 * keperluan pengujian otomatis dan basis data awal (seeding).
 *
 * @extends Factory<Penilaian>
 */
class PenilaianFactory extends Factory
{
    /**
     * Nama model yang bersesuaian dengan factory ini.
     *
     * @var class-string<Penilaian>
     */
    protected $model = Penilaian::class;

    /**
     * Definisikan nilai bawaan (default) atribut model.
     *
     * @return array<string, mixed>
     */
    public function definition(): array
    {
        return [
            'proposal_id' => Proposal::factory(),
            'dosen_id' => Dosen::factory(),
            'skor' => fake()->numberBetween(1_000_000, 999),
            'komentar' => fake()->realText(200),
            'status_finalisasi' => fake()->boolean(20),
            'tanggal' => fake()->dateTimeBetween('-2 years', 'now')->format('Y-m-d'),
        ];
    }
}
