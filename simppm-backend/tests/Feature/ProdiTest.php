...
/**
 * Feature Test JSON:API untuk resource "prodi" (Program Studi).
 *
...
class ProdiTest extends TestCase
{
    ...
    {
        $response = $this->getJson('/api/v1/prodi', $this->headerJsonApi());
     ...
     * pada seluruh kategori entitas SIMPPM (lihat ProdiPolicy::create()).
     */
    public function test_pengelola_dapat_membuat_data(): void
    {
        $this->sebagaiPeran(User::PERAN_ADMIN_LPPM);

        $response = $this->postJson('/api/v1/prodi', $this->payloadValid(), $this->headerJsonApi());
     ...
     * data "prodi" (lihat ProdiPolicy::create()), sehingga
    ...
    public function test_pengelola_dapat_melihat_detail(): void
    {
        ...
        $dibuat = $this->postJson('/api/v1/prodi', $this->payloadValid(), $this->headerJsonApi());
        $dibuat->assertStatus(201);
        $id = $dibuat->json('data.id');

        $response = $this->getJson("/api/v1/prodi/{$id}", $this->headerJsonApi());
