...
/**
 * Feature Test JSON:API untuk resource "pedoman" (Pedoman dan Kode Etik).
 *
...
class PedomanTest extends TestCase
{
    ...
    {
        $response = $this->getJson('/api/v1/pedoman', $this->headerJsonApi());
     ...
     * pada seluruh kategori entitas SIMPPM (lihat PedomanPolicy::create()).
     */
    public function test_pengelola_dapat_membuat_data(): void
    {
        $this->sebagaiPeran(User::PERAN_ADMIN_LPPM);

        $response = $this->postJson('/api/v1/pedoman', $this->payloadValid(), $this->headerJsonApi());
     ...
     * data "pedoman" (lihat PedomanPolicy::create()), sehingga
    ...
    public function test_pengelola_dapat_melihat_detail(): void
    {
        ...
        $dibuat = $this->postJson('/api/v1/pedoman', $this->payloadValid(), $this->headerJsonApi());
        $dibuat->assertStatus(201);
        $id = $dibuat->json('data.id');

        $response = $this->getJson("/api/v1/pedoman/{$id}", $this->headerJsonApi());
