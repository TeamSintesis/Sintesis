...
/**
 * Feature Test JSON:API untuk resource "skema-pendanaan" (Skema Pendanaan).
 *
 ...
 */
class SkemaPendanaanTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function payloadValid(): array
    {
        return [
            'data' => [
                'type' => 'skema-pendanaan',
                'attributes' => [
            'namaSkema' => 'Contoh nama skema',
            'jenis' => 'internal',
            'plafonDana' => '5000000.00',
            'sumberDana' => 'Contoh sumber dana',
                ],
            ],
        ];
    }
    ...
    {
        $response = $this->getJson('/api/v1/skema-pendanaan', $this->headerJsonApi());
     ...
     * pada seluruh kategori entitas SIMPPM (lihat SkemaPendanaanPolicy::create()).
     */
    public function test_pengelola_dapat_membuat_data(): void
    {
        $this->sebagaiPeran(User::PERAN_ADMIN_LPPM);

        $response = $this->postJson('/api/v1/skema-pendanaan', $this->payloadValid(), $this->headerJsonApi());
     ...
     * data "skema-pendanaan" (lihat SkemaPendanaanPolicy::create()), sehingga
    ...
    public function test_pengelola_dapat_melihat_detail(): void
    {
        ...
        $dibuat = $this->postJson('/api/v1/skema-pendanaan', $this->payloadValid(), $this->headerJsonApi());
        $dibuat->assertStatus(201);
        $id = $dibuat->json('data.id');

        $response = $this->getJson("/api/v1/skema-pendanaan/{$id}", $this->headerJsonApi());
