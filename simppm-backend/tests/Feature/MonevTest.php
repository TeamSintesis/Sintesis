...
/**
 * Feature Test JSON:API untuk resource "monev" (Monitoring dan Evaluasi).
 *
...
class MonevTest extends TestCase
{
     ...
     * "monev" valid, lalu kembalikan sebagai array
    ...
    protected function payloadValid(): array
    {
        return [
            'data' => [
                ...
                'attributes' => [
            'catatan' => 'Contoh isi catatan untuk keperluan pengujian otomatis.',
            'status' => 'terjadwal',
                ],
            ],
        ];
    ...
    {
        $response = $this->getJson('/api/v1/monev', $this->headerJsonApi());
     ...
     * pada seluruh kategori entitas SIMPPM (lihat MonevPolicy::create()).
     */
    public function test_pengelola_dapat_membuat_data(): void
    {
        $this->sebagaiPeran(User::PERAN_ADMIN_LPPM);

        $response = $this->postJson('/api/v1/monev', $this->payloadValid(), $this->headerJsonApi());
     ...
     * data "monev" (lihat MonevPolicy::create()), sehingga
    ...
    public function test_pengelola_dapat_melihat_detail(): void
    {
        ...
        $dibuat = $this->postJson('/api/v1/monev', $this->payloadValid(), $this->headerJsonApi());
        $dibuat->assertStatus(201);
        $id = $dibuat->json('data.id');

        $response = $this->getJson("/api/v1/monev/{$id}", $this->headerJsonApi());
