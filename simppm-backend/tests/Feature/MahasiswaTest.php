...
/**
 * Feature Test JSON:API untuk resource "mahasiswa" (Mahasiswa).
 *
...
class MahasiswaTest extends TestCase
{
     ...
     * "mahasiswa" valid, lalu kembalikan sebagai array
    ...
    {
        $response = $this->getJson('/api/v1/mahasiswa', $this->headerJsonApi());
     ...
     * pada seluruh kategori entitas SIMPPM (lihat MahasiswaPolicy::create()).
     */
    public function test_pengelola_dapat_membuat_data(): void
    {
        $this->sebagaiPeran(User::PERAN_ADMIN_LPPM);

        $response = $this->postJson('/api/v1/mahasiswa', $this->payloadValid(), $this->headerJsonApi());
     ...
     * data "mahasiswa" (lihat MahasiswaPolicy::create()), sehingga
    ...
    public function test_pengelola_dapat_melihat_detail(): void
    {
        ...
        $dibuat = $this->postJson('/api/v1/mahasiswa', $this->payloadValid(), $this->headerJsonApi());
        $dibuat->assertStatus(201);
        $id = $dibuat->json('data.id');

        $response = $this->getJson("/api/v1/mahasiswa/{$id}", $this->headerJsonApi());
