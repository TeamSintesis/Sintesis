...
/**
 * Feature Test JSON:API untuk resource "sarana-prasarana" (Sarana dan Prasarana).
 *
 ...
 */
class SaranaPrasaranaTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function payloadValid(): array
    {
        return [
            'data' => [
                ...
                'attributes' => [
            'namaSarpras' => 'Contoh nama sarpras',
    ...
    {
        $response = $this->getJson('/api/v1/sarana-prasarana', $this->headerJsonApi());
     ...
     * pada seluruh kategori entitas SIMPPM (lihat SaranaPrasaranaPolicy::create()).
     */
    public function test_pengelola_dapat_membuat_data(): void
    {
        $this->sebagaiPeran(User::PERAN_ADMIN_LPPM);

        $response = $this->postJson('/api/v1/sarana-prasarana', $this->payloadValid(), $this->headerJsonApi());
     ...
     * data "sarana-prasarana" (lihat SaranaPrasaranaPolicy::create()), sehingga
    ...
    public function test_pengelola_dapat_melihat_detail(): void
    {
        ...
        $dibuat = $this->postJson('/api/v1/sarana-prasarana', $this->payloadValid(), $this->headerJsonApi());
        $dibuat->assertStatus(201);
        $id = $dibuat->json('data.id');

        $response = $this->getJson("/api/v1/sarana-prasarana/{$id}", $this->headerJsonApi());
