...
/**
 * Feature Test JSON:API untuk resource "dosen" (Dosen/Peneliti).
 *
...
class DosenTest extends TestCase
{
     ...
     * "dosen" valid, lalu kembalikan sebagai array
    ...
    protected function payloadValid(): array
    {
        return [
            ...
            'nama' => 'Contoh nama',
            'jabatanFungsional' => 'Contoh jabatan fungsional',
            'bidangKepakaran' => 'Contoh bidang kepakaran',
            'idSinta' => 'Contoh id sinta',
            'scopusId' => 'Contoh scopus id',
            'orcid' => 'Contoh orcid',
                ],
            ],
        ];
    ...
    {
        $response = $this->getJson('/api/v1/dosen', $this->headerJsonApi());
     ...
     * pada seluruh kategori entitas SIMPPM (lihat DosenPolicy::create()).
     */
    public function test_pengelola_dapat_membuat_data(): void
    {
        $this->sebagaiPeran(User::PERAN_ADMIN_LPPM);

        $response = $this->postJson('/api/v1/dosen', $this->payloadValid(), $this->headerJsonApi());
     ...
     * data "dosen" (lihat DosenPolicy::create()), sehingga
    ...
    public function test_pengelola_dapat_melihat_detail(): void
    {
        ...
        $dibuat = $this->postJson('/api/v1/dosen', $this->payloadValid(), $this->headerJsonApi());
        $dibuat->assertStatus(201);
        $id = $dibuat->json('data.id');

        $response = $this->getJson("/api/v1/dosen/{$id}", $this->headerJsonApi());
