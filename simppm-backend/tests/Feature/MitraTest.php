...
/**
 * Feature Test JSON:API untuk resource "mitra" (Mitra Eksternal).
 *
...
class MitraTest extends TestCase
{
    ...
    protected function payloadValid(): array
    {
        return [
            'data' => [
                'type' => 'mitra',
                'attributes' => [
            'namaMitra' => 'Contoh nama mitra',
            'jenisMitra' => 'industri',
            'kontak' => 'Contoh kontak',
    ...
    {
        $response = $this->getJson('/api/v1/mitra', $this->headerJsonApi());
     ...
     * pada seluruh kategori entitas SIMPPM (lihat MitraPolicy::create()).
     */
    public function test_pengelola_dapat_membuat_data(): void
    {
        $this->sebagaiPeran(User::PERAN_ADMIN_LPPM);

        $response = $this->postJson('/api/v1/mitra', $this->payloadValid(), $this->headerJsonApi());
     ...
     * data "mitra" (lihat MitraPolicy::create()), sehingga
    ...
    public function test_pengelola_dapat_melihat_detail(): void
    {
        ...
        $dibuat = $this->postJson('/api/v1/mitra', $this->payloadValid(), $this->headerJsonApi());
        $dibuat->assertStatus(201);
        $id = $dibuat->json('data.id');

        $response = $this->getJson("/api/v1/mitra/{$id}", $this->headerJsonApi());
