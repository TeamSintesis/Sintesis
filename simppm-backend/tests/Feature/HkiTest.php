...
/**
 * Feature Test JSON:API untuk resource "hki" (Kekayaan Intelektual).
 *
 ...
 */
class HkiTest extends TestCase
{
    use RefreshDatabase;
     ...
     * "hki" valid, lalu kembalikan sebagai array
    ...
    {
        $response = $this->getJson('/api/v1/hki', $this->headerJsonApi());
     ...
     * pada seluruh kategori entitas SIMPPM (lihat HkiPolicy::create()).
     */
    public function test_pengelola_dapat_membuat_data(): void
    {
        $this->sebagaiPeran(User::PERAN_ADMIN_LPPM);

        $response = $this->postJson('/api/v1/hki', $this->payloadValid(), $this->headerJsonApi());
     ...
     * data "hki" (lihat HkiPolicy::create()), sehingga
    ...
    public function test_pengelola_dapat_melihat_detail(): void
    {
        ...
        $dibuat = $this->postJson('/api/v1/hki', $this->payloadValid(), $this->headerJsonApi());
        $dibuat->assertStatus(201);
        $id = $dibuat->json('data.id');

        $response = $this->getJson("/api/v1/hki/{$id}", $this->headerJsonApi());
