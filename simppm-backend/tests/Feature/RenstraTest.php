...
/**
 * Feature Test JSON:API untuk resource "renstra" (Rencana Strategis).
 *
...
class RenstraTest extends TestCase
{
    ...
    {
        $response = $this->getJson('/api/v1/renstra', $this->headerJsonApi());
     ...
     * pada seluruh kategori entitas SIMPPM (lihat RenstraPolicy::create()).
     */
    public function test_pengelola_dapat_membuat_data(): void
    {
        $this->sebagaiPeran(User::PERAN_ADMIN_LPPM);

        $response = $this->postJson('/api/v1/renstra', $this->payloadValid(), $this->headerJsonApi());
     ...
     * data "renstra" (lihat RenstraPolicy::create()), sehingga
    ...
    public function test_pengelola_dapat_melihat_detail(): void
    {
        ...
        $dibuat = $this->postJson('/api/v1/renstra', $this->payloadValid(), $this->headerJsonApi());
        $dibuat->assertStatus(201);
        $id = $dibuat->json('data.id');

        $response = $this->getJson("/api/v1/renstra/{$id}", $this->headerJsonApi());
