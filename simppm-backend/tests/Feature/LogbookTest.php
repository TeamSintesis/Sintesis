...
/**
 * Feature Test JSON:API untuk resource "logbook" (Logbook Kemajuan).
 *
...
class LogbookTest extends TestCase
{
     ...
     * "logbook" valid, lalu kembalikan sebagai array
    ...
    protected function payloadValid(): array
    {
        return [
            ...
            'isiKemajuan' => 'Contoh isi isi kemajuan untuk keperluan pengujian otomatis.',
            'tanggalIsi' => '2026-01-15',
                ],
            ],
        ];
    }
    ...
    {
        $response = $this->getJson('/api/v1/logbook', $this->headerJsonApi());
     ...
     * pada seluruh kategori entitas SIMPPM (lihat LogbookPolicy::create()).
     */
    public function test_pengelola_dapat_membuat_data(): void
    {
        $this->sebagaiPeran(User::PERAN_ADMIN_LPPM);

        $response = $this->postJson('/api/v1/logbook', $this->payloadValid(), $this->headerJsonApi());
     ...
     * data "logbook" (lihat LogbookPolicy::create()), sehingga
    ...
    public function test_pengelola_dapat_melihat_detail(): void
    {
        ...
        $dibuat = $this->postJson('/api/v1/logbook', $this->payloadValid(), $this->headerJsonApi());
        $dibuat->assertStatus(201);
        $id = $dibuat->json('data.id');

        $response = $this->getJson("/api/v1/logbook/{$id}", $this->headerJsonApi());
